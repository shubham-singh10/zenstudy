...
const Articles = () => {
    ...
    const getarcticle = async () => {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API}zenstudy/api/post/getposts?page=${currentPage}&limit=${itemperpage}&category=${category}`,
          {
            method: "GET",
            headers: {
        ...
        // Set data immediately after it arrives
        setArticlePost(data.posts);
        setArticleTotal(data.totalPages);
      } catch (error) {
        setError(error);
      } finally {
        // Always set loading to false after data is processed
        setLoading(false);
      }
    };
  ...
  return (
    <div className="px-0 md:px-10 lg:px-12 m-10">
      <h1 className=" text-2xl mt-10 md:text-3xl lg:text-4xl mb-8 text-center text-[#054BB4] font-semibold">
        O<span className="border-b-8 border-[#054BB4]">ur Blog Pos</span>ts
      </h1>
      ...
      <div className="flex flex-col md:flex-row lg:flex-row justify-between gap-4 my-8 mx-50px">
        <div className="flex items-center bg-blue-100 rounded-full px-4 py-2 mb-4 w-full md:w-1/2 lg:w-1/2 ">
          ...
          <button className="text-blue-500">
            <FaSearch />{" "}
            {/* Assuming Font Awesome is included in the project */}
          </button>
        </div>
      ...
      <div className="space-y-4 mt-10 ">
        {filteredData.length === 0 ? (
          ...
          filteredData.map((post, index) => (
            <div
              key={index}
              className="flex flex-col md:flex-row bg-white rounded-lg shadow-md overflow-hidden mb-10 hover:shadow-lg transition duration-300"
            >
              <img
                src={post.image}
                alt={post.title}
                className="h-full w-full md:w-1/3 object-cover object-center"
              />
              <div className="p-4 flex flex-col justify-between flex-1">
                <div>
                  <h2 className="text-xl font-bold">{post.title}</h2>
                  <p className="text-gray-600 mt-2 text-justify">
                    {stripHtmlTags(he.decode(post.content)).substring(0, 500)}
                    ...
                  </p>
                ...
                <div className="flex justify-between items-center mt-4">
                  <button
                    className="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 flex justify-center gap-1"
                    onClick={() =>
